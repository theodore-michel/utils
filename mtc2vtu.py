...
#         num_cells += 1
num_points, num_cells = map(int, MESH_LINES[0].split()[::2])
...
with open(MTC_FILE, 'r') as MTC:
    MTC_LINES = MTC.readlines()[3:-1]

# Extract velocity vectors from the .mtc file
velocity_vectors = np.array([list(map(float, line.split())) for line in MTC_LINES])
...
with open(VTU_FILE, 'w') as vtu:
    ...
    for point_line in tqdm(MESH_LINES[1:num_points + 1]):
        x, y, z = map(float, point_line.split()[:3])
        vtu.write(f'{x} {y} {z}\n')
    ...
    for cell_line in tqdm(MESH_LINES[num_points + 2:num_points + 2 + num_cells]):          
        connectivities = list(map(int, cell_line.split()[:4]))
