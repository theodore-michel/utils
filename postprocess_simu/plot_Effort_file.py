...
IndexesMA = (int(WINDOWMA/DT),int(WINDOW[1]/DT))
...
def moving_avg(List, window=50):
    ...
    return(MAList)
CX_ma = moving_avg(CX,window=int(WINDOWMA/DT))
CY_ma = moving_avg(CY,window=int(WINDOWMA/DT))
...
ax1.plot(TIME[Indexes[0]:Indexes[1]],      CX[Indexes[0]:Indexes[1]],        color = 'lightgray', linewidth = 0.9, label = 'Instant',         linestyle='-')
ax1.plot(TIME[IndexesMA[0]:IndexesMA[1]],  CX_ma[IndexesMA[0]:IndexesMA[1]], color = 'black',     linewidth = 1,   label = 'Moving Average',  linestyle='-')
...
ax2.plot(TIME[Indexes[0]:Indexes[1]],      CY[Indexes[0]:Indexes[1]],        color = 'lightgray', linewidth = 0.9, label = 'Instant',        linestyle='-')
ax2.plot(TIME[IndexesMA[0]:IndexesMA[1]],  CY_ma[IndexesMA[0]:IndexesMA[1]], color = 'black',     linewidth = 1,   label = 'Moving Average', linestyle='-')
